...
| Destructive DB ops gated | #3 | **HARD** | `permissions.deny: Bash(*DROP DATABASE*)` in project `settings.json` — HARD unconditional deny (no token override possible). `RESTORE DATABASE` is NOT in deny; it is gated by `PreToolUse` hook `.claude/hooks/guard_destructive_db.py` — requires a single-use approval token at `agent-workspace/RESTORE-APPROVED.token` to proceed (token consumed after one use). `DROP DATABASE` is also caught by the hook as belt-and-suspenders, but the deny already blocks it unconditionally. |
...
**How enforced:**
- `permissions.deny: ["Bash(git add -A*)", "Bash(git add .*)"]` in project `.claude/settings.json` — **HARD, wired**. These two are truly unconditional: no legitimate exception exists for bare-sweep staging.
...
**How enforced:**
- `team-ceo-scribe` is REMOVED by design. Routine logging is replaced by the `claude agents` view + OTEL timeline. Tier-2 decisions are logged directly by the CEO to `logs/<prefix>/cross-repo.log` (one-liner, no agent needed). (SOFT — agent file still exists on disk and will be registered by Claude Code until deleted; do NOT spawn it; see M2 remediation note in GOAT-CEO-REWORK-REVIEW.md)
...
| CEO context window is filling mid-session | Write `MISSION.md` checkpoint; spawn a fresh overseer with reconstruction brief; do NOT hallucinate state from fading context | #6 |
