...
The OPTIONAL capabilities below (rubric, research-KB, strict mode, codebase-index) are seen and toggled via the **`/goat-ceo:features`** command — defaults in the committed `.claude/goat-features.json` (all OFF), overlaid by the gitignored `.claude/goat-features.local.json` (personal), then per-repo flags in `repo-registry.json`, then session sentinels in `agent-workspace/`. Each rubric enforcement (block/heal/degrade) and research action (capture/run/benchmark) is recorded to the gitignored `logs/rubric-enforcement.jsonl` / `logs/research.jsonl` via `scripts/log_capability.py` (+ the heal-gate's own inline append). `settings.json` sets `PYTHONUTF8=1` so rubric's non-ASCII CLI output is safe on the Windows console.
...
| Reviewer citation spans must resolve (anti-fabrication) | #4 | **HARD** | `SubagentStop` hook `.claude/hooks/check_span_validity.py` opens each `cited_spans` entry in an A/B reviewer's verdict and blocks the stop (exit 2) if a cited code span does not exist in the file (whitespace-normalized). Adopts rubric's mechanical span-check — catches a fabricated `file:line` the read-floor (`check_toolcall_audit`) cannot. Judge/critic/Reviewer-C exempt; fail-open; placeholder + prompt-echo guarded — live |
...
| Real-time rubric self-heal must be CAPPED (no `maxTurns` thrash) | #3 | **HARD (opt-in, target-repo)** | `.claude/hooks/rubric_heal_gate.py` (PostToolUse `Edit\|Write`, copied into a target repo that opts in) heals a blocking standards violation ≤2 cycles/file, then DEGRADES to advisory + logs `RUBRIC-DEGRADED.md` (and records each block/degrade to `logs/rubric-enforcement.jsonl`) rather than thrashing the implementer past `maxTurns:30`. Default flow omits it (CEO-run `RUBRIC.GATE` is the baseline). Fail-open. Design `§I` |
| Technical researcher CAPTURES every external source it consults (free) and reuses VERIFIED subjects before re-researching, when RESEARCH-KB-AVAILABLE | #8 | **SOFT** | `{RESEARCH_KB_STATUS}` block in templates §7 — capture-always (`run_capture`, no LLM) so the KB is comprehensive; reuse a VERIFIED subject (`synthesis.md` present) → cite + SKIP the online run; verify on demand (`run_research`) over already-captured sources. Design `§J`; ledger P14 |
| ONLY `verdict: supported` claims are pre-verified + SINGLE-SOURCE-exempt; CAPTURED-but-unverified sources are NOT | #4 | **SOFT** | a `synthesis.md`/`claims.jsonl` `supported` claim carries cross-model quote-match + 3-judge corroboration → cite `{slug, source_id, quote, verdict}`, not SINGLE-SOURCE. A subject with `sources/` but NO `claims.jsonl` (or verdict ≠ supported) is RAW UNVERIFIED material — verify it first or treat as an ordinary SINGLE-SOURCE finding. Certifies traceability-to-source, not truth (a faithful claim from a wrong source still passes — source quality is a human judgment) |
| Research System is OPT-IN, `claude -p`-cost, with NO `RESEARCH.GATE` | #2 | **SOFT** | additive context, not a correctness gate (feeds the existing 5-condition convergence gate); template §7 routes throwaway lookups to WebSearch/`deep-research`; gated to "worth persisting," cost-flagged. Design `§J.2` |
| Verified, sourced coding-standards from research MAY feed rubric (research → codify → human-approved proposals) | #4 | **SOFT** | the `§J.1`↔`§I` loop: makes rubric conventions evidence-backed; rubric never auto-merges, and faithfulness ≠ truth, so human approval is required |
...
| CEO context window is filling mid-session | Collaborative: refresh the `RESUME-STATE.md` body and continue — auto-compaction gives durable, machine-grounded resume (git state + sentinels + the machine-refresh block survive; running prose is capped and can decay), not a stop condition. Unattended: see `unattended-mode.md` §3. Do NOT hallucinate state from fading context. | #6, #3 |
