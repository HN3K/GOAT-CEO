...
| Technical researcher checks `research-kb/INDEX.md` before commissioning online research when RESEARCH-KB-AVAILABLE (reuse-before-research) | #8 | **SOFT** | `{RESEARCH_KB_STATUS}` block in templates §7: KB-hit → cite stored `synthesis.md`/`claims.jsonl` + SKIP the online run (saves time/tokens; compounds across sessions); KB-miss → research + capture into `research-kb/<slug>/`. Design `§J`; ledger P14 |
| Research-System findings are NOT flagged SINGLE-SOURCE (pre-verified) and certify traceability-to-source, not truth | #4 | **SOFT** | cross-model quote-match + 3-judge ensemble already corroborate; cite `{slug, source_id, quote, verdict}`. A claim faithfully grounded in a wrong source still passes — source quality stays a human judgment |
