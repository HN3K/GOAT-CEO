...
- **rubric (optional standards system):** `.rubric/` directory exists at the repo root AND the `rubric` CLI responds — run `rubric kb --kb .rubric/kb` (the `kb` probe prints the KB summary and exits 0). rubric is a HOST tool: it need not be installed inside the target repo, only on the operator's PATH — the repo-scoped commands (`check`/`context`/`measure`/`init`/`index`) take `--repo <path>`, but `rubric kb` does NOT (passing `--repo` to `kb` errors). rubric is **vendored at `GOAT-CEO/tools/rubric/`** — if the CLI is not yet on PATH, install it from there with `pip install -e "tools/rubric[gate,retrieval]"` (see `tools/rubric/VENDORED.md`).
...
> A) Bootstrap — run `rubric init --no-claude --repo <path>` (scaffolds `.rubric/tools.json` + the `.rubric/index/` reuse index + a git pre-commit hook; does NOT install rubric's own Claude Code hooks/skill — GOAT-CEO drives rubric via its CLI). **IMPORTANT: `rubric init` creates NO conventions KB** — `.rubric/kb` starts absent/empty, so `rubric check` enforces only tool-default linters (e.g. Ruff) until you author it. To actually enforce standards/conventions you MUST populate `.rubric/kb/{conventions,rules,exemplars}` — copy + adapt the bundled TS-flavored starter at `tools/rubric/kb`, or author rules for this repo's language — AND put each rule's backing analyzer on PATH (`ast-grep` for structural `rules`; the `tools.json` linters for theirs). Only deterministic `rules` block; bare conventions/exemplars are advisory grounding (`rubric context`) only.
...
- **When RUBRIC-AVAILABLE:** Overseer ALSO spawns `{prefix}-reviewer-c` in parallel (`team-verifier`, tools `Read, Grep, Bash(rubric *)`, template §12a) — runs rubric's OWN verification (`rubric enforce --verify`: deterministic gate + grounded review + span-check + 3-judge ensemble) as the STANDARDS / conventions / reuse lens that A/B do not cover. Naturally exempt from `check_toolcall_audit` (its `reviewer:"C"` marker is not A/B). Skipped entirely when RUBRIC-UNAVAILABLE.
- Reviewer A: correctness perspective; Reviewer B: test-quality perspective; Reviewer C (when present): standards/conventions/reuse
- Both write independent JSON verdict blocks to `agent-workspace/REVIEW-LOG.md`
- **After the reviewer verdict blocks (A and B, plus Review C when RUBRIC-AVAILABLE) are present in `REVIEW-LOG.md`** (not before, and NOT conditioned on `REVIEW.GATE`): the Overseer spawns the completeness-critic and then the judge. This is the correct execution order — `REVIEW.GATE` is written by the judge PASS, so critic and judge must run BEFORE the gate exists, not after. The critic reads A/B `criteria_checked`; the judge additionally reads Review C (rubric standards lens) and composes per templates §14.
