...
> ledger: P13. Source: the first-party `rubric` project, vendored under `tools/rubric/` (pin a commit before depending on it — built 2026-06-14/15,
...
| 2 | `goat-ceo.md §1.2` intake | detect `.rubric/` + `rubric kb --kb .rubric/kb` responds → record status; bootstrap offer A/B/C where A = `rubric init --no-claude` (scaffolds `.rubric/`, NOT a conventions KB — seed via `/goat-ceo:features → rubric seed`). `ro-reference` repos are EXEMPT | the Codebase-Index detect + A/B/C bootstrap |
